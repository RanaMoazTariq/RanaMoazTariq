<!-- PROFILE HEADER -->
<div align="center">
  <a href="https://moaztariq.online">
    <img src="https://capsule-render.vercel.app/api?type=waving&color=00FF00&height=200&section=header&text=Moaz%20Tariq&fontSize=60&fontColor=ffffff&animation=fadeIn" alt="Header" />
  </a>

<!-- TYPING ANIMATION -->
  <p align="center">
    <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=26&duration=4000&pause=1000&color=00FF00&center=true&vCenter=true&width=500&height=50&lines=Full-Stack+Developer;AI+Engineer;Tech+Entrepreneur;Open+Source+Contributor" alt="Typing Animation">
  </p>

<!-- SOCIAL BADGES -->
  <p align="center">
    <a href="[https://linkedin.com/in/www.linkedin.com/in/ranamoaztariq](https://www.linkedin.com/in/ranamoaztariq/)">
      <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" />
    </a>
    <a href="mailto:moazworks@gmail.com">
      <img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white" />
    </a>
    <a href="https://wa.me/03121720039">
      <img src="https://img.shields.io/badge/WhatsApp-25D366?style=for-the-badge&logo=whatsapp&logoColor=white" />
    </a>
  </p>
</div>

## 🔥 About Me

- 👨‍💻 I’m @RanaMoazTariq — Founder of **Prime Layer Technologies**
- 🌍 Based in Pakistan — working globally 🌐
- 🧠 I build AI-powered tools, web applications, and mobile apps
- 📱 Also passionate about graphic design, SEO, and digital branding
- ⚙️ Tech Stack: **MERN**, **PHP**, **MySQL**, **Bootstrap**, **AI APIs**, **MongoDB**, **Firebase**, **React Native**
- 🛠 Tools: Figma, Canva, Adobe Illustrator, Postman, Git, VS Code, Android Studio

---

## 🚀 Expertise

- 🧩 Full Stack Web Development (MERN, PHP, MySQL)
- 📱 Mobile Application Development (Android, React Native)
- 🤖 AI-Powered Tools & Platforms
- 🎨 Graphic Design & Branding (Logos, Banners, UI)
- 📈 SEO & Digital Marketing
- 🔐 Cybersecurity Awareness & Implementation
- 📊 Database Design (MySQL, MongoDB)

---

## 🌱 Currently Learning

- Advanced Backend (Laravel, Node.js, MongoDB)
- AI/ML integrations (OpenAI, LangChain)
- DevOps & CI/CD
- Cloud Deployment (VPS, cPanel, Firebase)

---

## 🤝 Let's Collaborate On

- 🔧 Open-source web tools
- 🌐 Educational platforms and LMS
- ⚙️ Automation tools & SaaS
- 📊 AI-powered MCQ platforms, resume generators, debuggers, and more

---

## 📫 Contact Me

- 📧 **Email:**  (moazworks@gmail.com) & (contact@moaztariq.online)
- 🌐 **Portfolio:** [moaztariq.online](https://moaztariq.online)  
- 💼 **LinkedIn:** [(https://www.linkedin.com/in/ranamoaztariq/)](https://www.linkedin.com/in/moaz-tariq-b98a1834b/)
- 💻 **Website:** [Prime Layer Technologies](#) (Coming Soon)

---

## 🛠️ Languages & Tools

<p align="left">
  <img src="https://skillicons.dev/icons?i=html,css,js,bootstrap,react,nodejs,mongodb,php,mysql,firebase,figma,androidstudio,github,vscode,ai" />
</p>

---


---

## 😄 Fun Fact

> I started a tech company during my studies, created over **10+ AI tools**, and continue to help others build digital solutions from scratch 🚀

---

